...
class BcSedeContratoController extends ControllerBase
{
  ...
  public function crearBeneficiarioAction()
  {
      ...
      $beneficiarios_pendientes  = CobOferentePersonaSimat::findFirst([
        'columns'    => '*',
        'conditions' => 'estado_certificacion = ?1 AND estado_activo = ?2 AND id_contrato = ?3',
        'bind'       => [
                1 => '2',
                2 => 1,
                3 => $id_contrato,
      ...
      if (!empty($beneficiario_simat_pendiente)) {
        $this->flash->error("El documento ". $documento ." ya se encuentra registrado en su contrato y se encuentra pendiente por aprobación de la matricula");
        return $this->response->redirect("bc_sede_contrato/beneficiarios");
      }
...
//--------------------Crud Sede Contrato -----------------------------
  ...
  public function guardarSedeContratoAction()
  {
      ...
      $modalidad = BcOferente::findFirst(['id_contrato' => $this->request->getPost("id_contrato")]);
  ...
  public function exportar_beneficiariosAction()
  {
    $beneficiarios = CobOferentePersonaSimat::find(['estado_activo in (1,2)']);
    $this->view->beneficiarios = $beneficiarios;
    $this->assets
    ->addJs('js/jquery.tablesorter.min.js')
    ->addJs('js/jquery.tablesorter.widgets.js')
    ...
    ->addJs('js/xlsx.core.min.js')
    ->addJs('js/reporte.js');
    $this->view->setTemplateAfter('../bc_sede_contrato/exportar_beneficiarios_activos');
  }

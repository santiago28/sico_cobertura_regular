...
<table class="table table-bordered table-hover" id="permisos_lista">
    ...
    {% for permiso in permisos %}
        <tr data-estado="{{ permiso.estado }}" class="bg-{{ permiso.getEstadoStyle() }}">
            <td><a rel="tooltip" title="Ver Detalles del Permiso" href="{{ url("bc_permiso/permiso/"~permiso.id_permiso) }}"><?php if ($permiso->estado == 0 || $permiso->estado == 3){ ?><a href="/sico_cobertura_regular/bc_permiso/aprobar/<?php echo $permiso->id_permiso; ?>" rel="tooltip" title="Pre Aprobar"><i class="glyphicon glyphicon-ok"></i></a> <?php } ?><?php if ($permiso->estado == 0 || $permiso->estado == 1){ ?><a href="#eliminar_elemento" rel="tooltip" title="Anular" class="eliminar_fila" data-id = "{{ permiso.id_permiso }}" data-toggle = "modal" id="{{ url("bc_permiso/eliminar/"~permiso.id_permiso) }}"><i class="glyphicon glyphicon-remove"></i></a> <?php } ?>{{ permiso.id_permiso }}</a></td>
